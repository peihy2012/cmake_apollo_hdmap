...
inline std::string StrCat(const std::string& str1) {
  return (str1);
}
...
inline std::string StrCat(const std::string& str1, const std::string& str2, 
    const std::string& str3, const std::string& str4, const std::string& str5) {
...
inline bool EndWith(const std::string& ori, const std::string& pat) {
  ...
  int endlen = pat.size();
  if(srclen >= endlen) {
    std::string temp = ori.substr(srclen - endlen, endlen);
    if(temp == pat)
      return true;
  }
  return false;
...
inline bool StartWith(const std::string& ori, const std::string& pat) {
  ...
  int srclen = ori.size();
  int startlen = pat.size();
  if(srclen >= startlen) {
    std::string temp = ori.substr(0, startlen);
    if(temp == pat)
      return true;
  }
  return false;
  ...
  if (transformer) {
    for (auto iter = begin; iter != end; ++iter) {
      if (iter == begin) {
        ...
        result = result + transformer(*iter);
      } else {
        // StrAppend(&result, delimiter, transformer(*iter));
        result = result + delimiter + transformer(*iter);
      }
    }
  } else {
