...
inline std::string EndWayPointFile() {
  if (FLAGS_use_navigation_mode) {
    return apollo::common::util::StrCat(
        std::string(FLAGS_navigation_mode_end_way_point_file.c_str()));
  } else {
    return apollo::common::util::StrCat(std::string(FLAGS_map_dir.c_str()), "/",
                            std::string(FLAGS_end_way_point_filename.c_str()));
  }
}

inline std::string SpeedControlFile() {
  return apollo::common::util::StrCat(std::string(FLAGS_map_dir.c_str()), "/",
                            std::string(FLAGS_speed_control_filename.c_str()));
}
